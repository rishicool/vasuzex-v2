...
/**
 * CleanupTestServer - Extends BaseServer
 */
class CleanupTestServer extends BaseServer {
  constructor() {
    // Get project root (2 levels up from apps/{name}/{type})
    const projectRoot = path.resolve(process.cwd(), '../..');
    
    super({
      appName: process.env.APP_NAME || 'cleanup-test-api',
  ...
  async initializeDatabase() {
    try {
      ...
      const dbUrl = process.env.DATABASE_URL || 'postgresql://postgres@localhost:5432/cleanup-test_db';
      ...
      capsule.addConnection({
        ...
        database: process.env.POSTGRES_DB || 'cleanup-test_db',
      ...
      console.log('[Cleanup-testAPI] 🗄️  Database connected');
    } catch (error) {
      console.error('[Cleanup-testAPI] ❌ Database connection failed:', error.message);
      throw error;
    }
  }

  /**
   * Initialize cleanup-test-specific services
   */
  async initializeServices() {
    ...
    console.log('[Cleanup-testAPI] 📦 Services initialized');
  }
...
// Start the server
const server = new CleanupTestServer();
server.start();
