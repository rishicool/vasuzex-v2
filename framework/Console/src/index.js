...
class BlogServer extends BaseServer {
  constructor() {
    ...
    super({
      appName: process.env.APP_NAME || 'blog-api',
  ...
  async initializeDatabase() {
    try {
      ...
      const dbUrl = process.env.DATABASE_URL || 'postgresql://postgres@localhost:5432/blog_db';
      ...
      console.log('[BlogAPI] 🗄️  Database connected');
    } catch (error) {
      console.error('[BlogAPI] ❌ Database connection failed:', error.message);
      throw error;
    }
  }

  /**
   * Initialize blog-specific services
   */
  async initializeServices() {
    ...
    console.log('[BlogAPI] 📦 Services initialized');
  }
